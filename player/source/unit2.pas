...
procedure TForm2.Ed_ipChange(Sender: TObject);
begin
  net_change:=true;
end;

procedure TForm2.Ed_portChange(Sender: TObject);
begin
  net_change:=true;
end;
...
begin
  ...
  receiverThread.WaitFor;
  receiverthread.free;
  receiverthread := TReceiverThread.Create(True);
...
begin
  ...
  begin
    ...
    if MessageDlg('Delete Device', 'Do you really want to delete "' + s + '"?' + LineEnding + 'If the device still exists, it will be recreated with default values on next start.', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      closealsa;
